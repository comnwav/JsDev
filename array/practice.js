...
objAry = JSON.parse(data); // 문자열 => Object 변경
console.log(objAry);

// filter() : 반환조건 참인 요소를 가지고 새로운 배열
let newAry = objAry.filter(e => {
...
function femaleList(ary) {
    ...
    ary.forEach(function (e) {
        tag += `<li>`;
        for ( let field in e) {
            tag += `${field}: ${e[field]} ${field=='ip_address'?'':', '}`;
        }
        tag += `</li>`;
    });
    tag += `</ul>`;
...
function femaleList2(ary) {
    let fmTag = `<table border="1"><thead><tr>`;
    
    for (let f in ary[0]) {
        fmTag += `<th>${f}</th>`;
    }
    fmTag += `</tr></thead><tbody>`;
